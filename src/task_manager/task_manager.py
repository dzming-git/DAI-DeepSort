...
class TaskInfo:
    def __init__(self, task_id: int):
        ...
        self.__target_detection_address: List[str, str] = []
        self.__target_detection_client: TargetDetectionClient = None
        self.target_label: str = 'person'  # 默认跟踪person

        self.__weights_folder = config.weights_folder
        self.__weight: str = 'ckpt.t7'
        self.__device_str: str = ''
        self.__max_tracking_length: int = 10
    ...
    def set_pre_service(self, pre_service_name: str, pre_service_ip: str, pre_service_port: str, args: Dict[str, str]):
        ...
        if 'target detection' == pre_service_name:
            self.__target_detection_address = [pre_service_ip, pre_service_port]
            self.__target_detection_client = TargetDetectionClient(pre_service_ip, pre_service_port, self.id)
    ...
    def set_cur_service(self, args: Dict[str, str]):
        ...
        if 'MaxTrackingLength' in args:
            self.__max_tracking_length = int(args['MaxTrackingLength'])
    
    def check(self) -> Tuple[bool, str]:
        if not self.__target_detection_client:
    ...
    def start(self) -> None:
        ...
        builder.device_str = self.__device_str
        builder.weights = f'{self.__weights_folder}/{self.__weight}'
        builder.max_tracking_length = self.__max_tracking_length
        self.tracker = builder.build()
        self.__target_detection_client.filter.clear()
        target_label_id = self.__target_detection_client.query_label_id(self.target_label)
        self.__target_detection_client.filter.add(target_label_id)
        self.__stop_event.clear()  # 确保开始时事件是清除状态
        self.__track_thread = threading.Thread(target=self.track_by_image_id)
    ...
    def track_by_image_id(self):
        while not self.__stop_event.is_set():  # 使用事件来检查停止条件
            ...
            try:
                ...
                if self.__stop_event.is_set():  # 在可能的长时间操作之前再次检查
                    break
                ...
                if self.__stop_event.is_set():  # 在可能的长时间操作之前再次检查
                    break
                results = self.__target_detection_client.get_result_by_image_id(image_id)
